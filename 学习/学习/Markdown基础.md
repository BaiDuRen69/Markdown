...
Markdown 的链接语法 `[文本](路径)` 本质上只是一个**超链接**，可以链接任何类型的文件（如 `.pdf`、`.docx`、`.png`、`.zip` 、`.txt`等）

跳转链接可以和锚点链接一起使用  
[jjf](Obsidian使用指南.md#坚果云同步)
...
`[展示文本](../文件名)`  
[试试看，跳转至README](../../README.md)

3.同级其他目录

`[展示文本](../同级其他目录/文件名)`  
[试试看，跳转至随记中的《荒诞》](../../随记/荒诞.md)

4.跨多级目录

`[](../../目录/目录/文件名)`  
步骤：
1. **找共同祖先目录**：从当前文件和目标文件分别往上找，找到第一个相同的父目录
2. **数当前文件到共同祖先的层数**：有几个中间目录就写几个 `../`
3. **拼接从共同祖先到目标文件的下行路径**

##### 绝对路径

大部分平台不适用，不建议使用（以下仅以Obsidian为例）
...
![示例图片](assets/Markdown基础/file-20260725122457463.jpg)
...
[常见的Markdown 图标语法](常见的Markdown%20图标语法.md)
